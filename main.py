...
class App:
    ...
    def generate_excel(self):
        ...
        query = text(f"""
            ...
            WHERE TRY_CONVERT(DATETIME, Date_Time, {date_style}) >= TRY_CONVERT(DATETIME, :s, {date_style})
            AND TRY_CONVERT(DATETIME, Date_Time, {date_style}) <= TRY_CONVERT(DATETIME, :e, {date_style})
            ORDER BY TRY_CONVERT(DATETIME, Date_Time, {date_style}) DESC
        """)
    ...
    def show_about(self):
        ...
        try:
            ...
            tk.Label(top, image=logo).pack(pady=5)
            top.logo = logo
        except:
            pass

        tk.Label(top, text="Premium Industrial Solutions Pvt. Ltd.", font=("Arial", 10, "bold")).pack()
        tk.Label(top, text="Software: Database Query to Excel").pack()
        tk.Label(top, text="Project: BBPL KINLEY RO & MIS").pack()
        tk.Label(top, text="Version: 1.0").pack()
        tk.Label(top, text="Updated By Sekh Sahabaj").pack()
        tk.Label(top, text="Developed by: Subhajit Duttagupta").pack(pady=5)
